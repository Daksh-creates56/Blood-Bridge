...
import { initialDonors } from '@/lib/data';
  ...
  const columns: ColumnDef<Donor>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => {
          ...
          if (bloodType.includes('O')) variant = 'destructive';
          if (bloodType.includes('AB')) variant = 'default';
          return <Badge variant={variant}>{bloodType}</Badge>
      },
    },
    {
    ...
    },
    {
      ...
      cell: ({ row }) => <div>{formatDistanceToNow(new Date(row.getValue('lastDonation')), { addSuffix: true })}</div>,
    },
    {
